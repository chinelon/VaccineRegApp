...
import { createVaccineEntry, deleteVaccineEntry, getDbConnection, getVaccineEntries, transformEntriesToDateSections } from './src/services/vaccine-entry.service';
import { Button, Icon, Text } from 'react-native-elements';
import AddEntry from './src/components/AddEntry';
import { DisplayOptions, ISettings, IState, IVaccineEntry } from './src/interfaces/vaccine-entry.interface';
...
const App: React.FC = () => {
   ...
   */
  const createEntry = (vaccineEntryData: IVaccineEntry) => {
    createVaccineEntry(vaccineEntryData, state, setState);
  }
  ...
  const deleteEntry = (id: number) => {
    deleteVaccineEntry(id, state, setState);
  }
  ...
  const getEntriesInDateSections = useMemo(() => {
    return transformEntriesToDateSections(state.vaccineEntries)
  ...
  const displayEntries = () => {
    ...
    switch (settings.displayOption) {
      case DisplayOptions.FLAT_LIST: return <EntryFlatList entries={state.vaccineEntries} deleteEntry={deleteEntry} />
      case DisplayOptions.SPREADSHEET: return <Spreadsheet entries={state.vaccineEntries} deleteEntry={deleteEntry} />
  ...
  return (
    <SafeAreaView style={styles.container}>
      <Text h2>Vaccinated Clients</Text>
